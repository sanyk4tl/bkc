 ...
 *****************************************************************************
 *  module: bkc_sandbox
 *  file name: bkc_sandbox_tspf.c
 *  description: supporting utilites for the bkc_sandbox_test mod
 *****************************************************************************
 *  log:
 *  >>>
 *  version:
...
#include <stdio.h>
#include <stdlib.h>
#include "bkc_sandbox_tspf.h"
/*********************definition, types & constants *************************/
/*1000k blocks*/
#define  BKC_SANDBOX_TSPF_MAX_BLOCK_NUM   (1024000)
/*1024Gb per block*/
#define  BKC_SANDBOX_TSPF_BLOCK_SIZE   (1024 * 1024 * 1024 * 1024)
/*********************variables**********************************************/
/*for memory diagnose*/
static void *mem_pool[BKC_SANDBOX_TSPF_MAX_BLOCK_NUM] = {NULL};
/*********************prototype of local functions***************************/

/*********************implementation of open and local functions*************/

/*****************************************************************************
 *                  bkc_sandbox_tspf_memdgn
 *****************************************************************************
 * parameters:
 * return value:
 * description:
 ...
 *****************************  Notes  *****************************
 ****************************************************************************/
void bkc_sandbox_tspf_memdgn(bkc_sandbox_tspf_malloc_func_t malloc_func,
        bkc_sandbox_tspf_free_func_t free_func)
{
    int loop = 0;
    ...
    if (malloc_func == NULL || free_func == NULL) {
        ...
        printf("=============>begin bkc_sandbox_tspf_memdgn\n");
    }

    block_size = BKC_SANDBOX_TSPF_MAX_BLOCK_NUM;

    for (loop = 0; loop < BKC_SANDBOX_TSPF_MAX_BLOCK_NUM; loop++)
        mem_pool[loop] = NULL;

    loop = 0;
    while (loop < BKC_SANDBOX_TSPF_MAX_BLOCK_NUM) {
    ...
    printf("=============>end bkc_sandbox_tspf_memdgn\n");
    return;
}
