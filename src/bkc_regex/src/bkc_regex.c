 ...
 *     is in debugging mode in which BKC_REGEX_SPF_RELEASE is not
 *     defined in bkc_regex_spf.h. if BKC_REGEX_SPF_RELEASE is defined
 *     in bkc_regex_spf.h, this func will always show empty messages
 *
 ...
 *     this func can be temporarily export to bkc_regex.h for outer
 *     refering
 *****************************  Notes  *****************************
 ************************************************************************/
int bkc_regex_watchmm(void)
{
    ...
    BKC_FLOWCONTROL_TRY {
        BKC_CR = bkc_regex_spf_watchmm();
        BKC_FC_CKSTHR(BKC_CR == BKC_REGEX_RSUC, 1, BKC_REGEX_RERR, NULL);
    }
