 ...
 *************************************************************************
 *  module: bkc_queue
 *  file name: bkc_queue_tspf.c
 *  description: supporting utilites for the bkc_queue_test mod
 *************************************************************************
 *  log:
 *  >>>
 *  version: 
...
/*1000k blocks*/
#define  BKC_QUEUE_TSPF_MAX_BLOCK_NUM   (1024000)
/*1024Gb per block*/
#define  BKC_QUEUE_TSPF_BLOCK_SIZE   (1024 * 1024 * 1024 * 1024)
/*********************variables******************************************/
/*for memory diagnose*/
static void *mem_pool[BKC_QUEUE_TSPF_MAX_BLOCK_NUM] = {NULL};
/*********************prototype of local functions***********************/

/*********************implementation of open and local functions*********/

/*************************************************************************
 *                  bkc_queue_tspf_memdgn
 *************************************************************************
 * parameters:
 * return value:
 * description:
 ...
 *****************************  Notes  *****************************
 ************************************************************************/
void bkc_queue_tspf_memdgn(bkc_queue_tspf_malloc_func_t malloc_func, 
        bkc_queue_tspf_free_func_t free_func)
{
    int loop = 0;
    ...
    if (malloc_func == NULL || free_func == NULL) {
        ...
        printf("=============>begin bkc_queue_tspf_memdgn\n");
    }

    block_size = BKC_QUEUE_TSPF_MAX_BLOCK_NUM;

    for (loop = 0; loop < BKC_QUEUE_TSPF_MAX_BLOCK_NUM; loop++)
        mem_pool[loop] = NULL;

    loop = 0;
    while (loop < BKC_QUEUE_TSPF_MAX_BLOCK_NUM) {
    ...
    printf("=============>end bkc_queue_tspf_memdgn\n");
    return;
}
