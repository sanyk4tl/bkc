 ...
 *     is in debugging mode in which BKC_SOCKET_SPF_RELEASE is not
 *     defined in bkc_socket_spf.h. if BKC_SOCKET_SPF_RELEASE is defined
 *     in bkc_socket_spf.h, this func will always show empty messages
 *
 ...
 *     this func can be temporarily export to bkc_socket.h for outer
 *     refering
 ***************************** notes **************************************
 ****************************************************************************/
int bkc_socket_watchmm(void)
{
    BKC_DITT(BKC_SOCKET_RSUC, 0);

    BKC_FLOWCONTROL_TRY {
        BKC_CR = bkc_socket_spf_watchmm();
        BKC_FC_CT(BKC_CR == BKC_SOCKET_RSUC, 1, BKC_SOCKET_RERR, NULL);
    }
