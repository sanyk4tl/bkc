 ...
 *****************************************************************************
 *  module: bkc_crc
 *  file name: bkc_crc_tspf.c
 *  description: supporting utilites for the bkc_crc_test mod
 *****************************************************************************
 *  log:
 *  >>>
 *  version:
...
#include <stdio.h>
#include <stdlib.h>
#include "bkc_crc_tspf.h"
/*********************definition, types & constants *************************/
/*1000k blocks*/
#define  BKC_CRC_TSPF_MAX_BLOCK_NUM   (1024000)
/*1024Gb per block*/
#define  BKC_CRC_TSPF_BLOCK_SIZE   (1024 * 1024 * 1024 * 1024)
/*********************variables**********************************************/
/*for memory diagnose*/
static void *mem_pool[BKC_CRC_TSPF_MAX_BLOCK_NUM] = {NULL};
 ...
 *****************************  Notes  *****************************
 ****************************************************************************/
void bkc_crc_tspf_memdgn(bkc_crc_tspf_malloc_func_t malloc_func,
        bkc_crc_tspf_free_func_t free_func)
{
    int loop = 0;
    ...
    if (malloc_func == NULL || free_func == NULL) {
        ...
        printf("=============>begin bkc_crc_tspf_memdgn\n");
    }

    block_size = BKC_CRC_TSPF_MAX_BLOCK_NUM;

    for (loop = 0; loop < BKC_CRC_TSPF_MAX_BLOCK_NUM; loop++)
        mem_pool[loop] = NULL;

    loop = 0;
    while (loop < BKC_CRC_TSPF_MAX_BLOCK_NUM) {
    ...
    printf("=============>end bkc_crc_tspf_memdgn\n");
    return;
}
