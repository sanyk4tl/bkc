...
/*400 blocks*/
#define  BKC_STORE_SPF_MAX_BLOCK_NUM   400
/*16 M per block*/
#define  BKC_STORE_SPF_BLOCK_SIZE   (1024 * 1024 * 16)
/*********************variables**********************************************/
...
/*for memory diagnose*/
static void *mem_pool[BKC_STORE_SPF_MAX_BLOCK_NUM] = {NULL};
...
void *bkc_store_spf_imalloc(unsigned int size)
{
    ...
    ret_p = bkc_store_spf_stdmalloc(size + sizeof(unsigned int) * 2);
    ...
    *malloc_block_p = (unsigned int )ret_p ;
    ...
    BKC_STORE_SPF_MDPRT("(malloc size is %d) \n", *malloc_num_p );
    BKC_STORE_SPF_MDPRT("(malloc addr is 0x%x) \n", (int)malloc_block_p);
...
int bkc_store_spf_ifree(void *p)
{
    ...
    if (NULL !=  p) {
        ...
        BKC_STORE_SPF_MDPRT("(free: size is %d) \n", *malloc_num_p );
        BKC_STORE_SPF_MDPRT("(free: addr is 0x%x)\n", (int)malloc_block_p);
        bkc_store_spf_stdfree(ret_p);
    }

    return BKC_STORE_SPF_RSUC;
}
...
void *bkc_store_spf_istrdup(char *str_p)
{
    ...
    if (str_p != NULL) {
        unsigned int len = 0;
        len = BKC_STORE_SPF_STD_STRLEN(str_p);
        retstr_p = bkc_store_spf_imalloc(len + 1);
        if (retstr_p != NULL) {
            BKC_STORE_SPF_STD_MEMSET(retstr_p, '\0', len + 1);
            BKC_STORE_SPF_STD_STRNCPY(retstr_p, str_p, len);
        }
    }
    return retstr_p;
}
...
{
#ifdef BKC_STORE_SPF_RELEASE
    BKC_STORE_SPF_ABSPRT("empty message: not in debugging mode.\n"
        "this functionality can be activated by undefining \n"
        "BKC_STORE_SPF_RELEASE in bkc_store_spf.h");
    return BKC_STORE_SPF_RSUC;
#else
    BKC_STORE_SPF_PRT("#####################\n");
    BKC_STORE_SPF_PRT("mem heap size is %u\n", hpsize);
    BKC_STORE_SPF_PRT("mem max use size is %u\n", maxhpsize );
    BKC_STORE_SPF_PRT("#####################\n");
    return BKC_STORE_SPF_RSUC;
#endif
}
        ...
        bkc_store_spf_free_func_t free_func)
{
    ...
    if (malloc_func == NULL || free_func == NULL) {
        BKC_STORE_SPF_ABSPRT("malloc or free func is null ,"
            "please have a check\n");
    } else {
        BKC_STORE_SPF_ABSPRT("=====>begin bkc_store_spf_memdgn\n");
    }

    block_size = BKC_STORE_SPF_MAX_BLOCK_NUM;

    for (loop = 0; loop < BKC_STORE_SPF_MAX_BLOCK_NUM; loop++)
        mem_pool[loop] = NULL;

    loop = 0;
    while (loop < BKC_STORE_SPF_MAX_BLOCK_NUM) {
    ...
    BKC_STORE_SPF_ABSPRT("bkc_store_spf_memdgn:total malloc size is hex[%x],"
    ...
    BKC_STORE_SPF_ABSPRT("====>end bkc_store_spf_memdgn\n");
    return;
}
...
{
    unsigned char *ret_p = NULL;
    ret_p = BKC_STORE_SPF_STD_MALLOC(size);

    if (ret_p != NULL) {
        BKC_STORE_SPF_STD_MEMSET(ret_p, BKC_STORE_MAGIC_STUFF, size);
    } else {
        BKC_STORE_SPF_ABSPRT("malloc failed, please have a check\n");
    }
    return ret_p;
}
...
{
    if (NULL !=  p) {
        BKC_STORE_SPF_STD_FREE(p);
        return BKC_STORE_SPF_RSUC;
    } else {
        BKC_STORE_SPF_ABSPRT("free NULL pointer, please have a check\n");
        return BKC_STORE_SPF_RERR;
    }
}
