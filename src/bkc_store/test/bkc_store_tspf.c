 ...
 *****************************************************************************
 *  module: bkc_store
 *  file name: bkc_store_tspf.c
 *  description: supporting utilites for the bkc_store_test mod
 *****************************************************************************
 *  log:
 *  >>>
 *  version:
...
#include <stdio.h>
#include <stdlib.h>
#include "bkc_store_tspf.h"
/*********************definition, types & constants *************************/
/*1000k blocks*/
#define  BKC_STORE_TSPF_MAX_BLOCK_NUM   (1024000)
/*1024Gb per block*/
#define  BKC_STORE_TSPF_BLOCK_SIZE   (1024 * 1024 * 1024 * 1024)
/*********************variables**********************************************/
/*for memory diagnose*/
static void *mem_pool[BKC_STORE_TSPF_MAX_BLOCK_NUM] = {NULL};
        ...
        bkc_store_tspf_free_func_t free_func)
{
    ...
    if (malloc_func == NULL || free_func == NULL) {
        ...
        printf("=============>begin bkc_store_tspf_memdgn\n");
    }

    block_size = BKC_STORE_TSPF_MAX_BLOCK_NUM;

    for (loop = 0; loop < BKC_STORE_TSPF_MAX_BLOCK_NUM; loop++)
        mem_pool[loop] = NULL;

    loop = 0;
    while (loop < BKC_STORE_TSPF_MAX_BLOCK_NUM) {
    ...
    printf("=============>end bkc_store_tspf_memdgn\n");
    return;
}
