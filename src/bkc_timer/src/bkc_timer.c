 ...
 *     is in debugging mode in which BKC_TIMER_SPF_RELEASE is not
 *     defined in bkc_timer_spf.h. if BKC_TIMER_SPF_RELEASE is defined
 *     in bkc_timer_spf.h, this func will always show empty messages
 *
 ...
 *     this func can be temporarily export to bkc_timer.h for outer
 *     refering
 *****************************  Notes  ************************************
 ****************************************************************************/
int bkc_timer_watchmm(void)
{
    BKC_DITT(BKC_TIMER_RSUC, 0);

    BKC_FLOWCONTROL_TRY {
        BKC_CR = bkc_timer_spf_watchmm();
        BKC_FC_CT(BKC_CR == BKC_TIMER_RSUC, 1, BKC_TIMER_RERR, NULL);
    }
