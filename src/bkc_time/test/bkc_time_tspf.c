 ...
 *****************************************************************************
 *  module: bkc_time
 *  file name: bkc_time_tspf.c
 *  description: supporting utilites for the bkc_time_test mod
 *****************************************************************************
 *  log:
 *  >>>
 *  version:
...
#include <stdio.h>
#include <stdlib.h>
#include "bkc_time_tspf.h"
/*********************definition, types & constants *************************/
/*1000k blocks*/
#define  BKC_TIME_TSPF_MAX_BLOCK_NUM   (1024000)
/*1024Gb per block*/
#define  BKC_TIME_TSPF_BLOCK_SIZE   (1024 * 1024 * 1024 * 1024)
/*********************variables**********************************************/
/*for memory diagnose*/
static void *mem_pool[BKC_TIME_TSPF_MAX_BLOCK_NUM] = {NULL};
/*********************prototype of local functions***************************/

/*********************implementation of open and local functions*************/

/*****************************************************************************
 *                  bkc_time_tspf_memdgn
 *****************************************************************************
 * parameters:
 * return value:
 * description:
 ...
 *****************************  Notes  *****************************
 ****************************************************************************/
void bkc_time_tspf_memdgn(bkc_time_tspf_malloc_func_t malloc_func,
        bkc_time_tspf_free_func_t free_func)
{
    int loop = 0;
    ...
    if (malloc_func == NULL || free_func == NULL) {
        ...
        printf("=============>begin bkc_time_tspf_memdgn\n");
    }

    block_size = BKC_TIME_TSPF_MAX_BLOCK_NUM;

    for (loop = 0; loop < BKC_TIME_TSPF_MAX_BLOCK_NUM; loop++)
        mem_pool[loop] = NULL;

    loop = 0;
    while (loop < BKC_TIME_TSPF_MAX_BLOCK_NUM) {
    ...
    printf("=============>end bkc_time_tspf_memdgn\n");
    return;
}
